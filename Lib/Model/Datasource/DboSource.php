  ...
  public function create(Model $model, $fields = null, $values = null) {
    ...
    if ($this->execute($this->renderStatement('create', $query))) {
      if ($model->primaryKey !== false) {
        ...
        // informed in the corresponding array Model:$compositePrimaryKey
        //
        // After this, the primary keys that were found are set in the Model
        if (is_array($model->compositePrimaryKey) && (count($model->compositePrimaryKey) > 0)) {
          $ids = array();
          foreach($model->compositePrimaryKey as $mpk) {
