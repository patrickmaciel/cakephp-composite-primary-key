 ...
 */
  public function read($fields = null, $id = null) {
    ...
    if ($id !== null && $id !== false && !is_array($id)) {
      ...
      $id !== false && 
      is_array($id) &&
      (is_array($this->compositePrimaryKey) && (count($this->compositePrimaryKey) > 0))
    ) {

      $conditions = array();
      foreach($this->compositePrimaryKey as $mpk) {
 ...
 */
  public function save($data = null, $validate = true, $fieldList = array()) {
    ...
    if ($this->primaryKey !== false && empty($this->data[$this->alias][$this->primaryKey])) {
      ...
      (
        isset($this->compositePrimaryKey) &&
        is_array($this->compositePrimaryKey) &&
        (count($this->compositePrimaryKey) > 0)
      )
    ) {
